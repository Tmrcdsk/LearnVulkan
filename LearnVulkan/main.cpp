...
struct Vertex
{
	...
	static std::array<VkVertexInputAttributeDescription, 3> getAttributeDescriptions() {
		...
		attributeDescriptions[1].binding = 0; // ！！注意：这里的binding与上面一样都是 0 ！！
		attributeDescriptions[1].location = 1;
		attributeDescriptions[1].format = VK_FORMAT_R32G32B32_SFLOAT;
		attributeDescriptions[1].offset = offsetof(Vertex, color);

		attributeDescriptions[2].binding = 0; // ！！注意：这里的binding与上面一样都是 0 ！！
...
private:
	...
	void createSwapChain()
	{
		...
		createInfo.imageUsage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT; // imageUsage成员变量用于指定我们将在图像上进行怎样的操作
	...
	static VKAPI_ATTR VkBool32 VKAPI_CALL debugCallback(
		VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity, // 消息的级别
		VkDebugUtilsMessageTypeFlagsEXT messageType,
		const VkDebugUtilsMessengerCallbackDataEXT* pCallbackData,
		void* pUserData // 指向了我们设置回调函数时，传递的数据的指针
	) {
